window.matchMedia = window.matchMedia || function (doc, undefined) {
    ...
    if (win.respond = {}, respond.update = function () {
        ...
        var lastCall, resizeDefer, eminpx, doc = win.document, docElem = doc.documentElement, mediastyles = [], rules = [], appendedEls = [], parsedSheets = {}, resizeThrottle = 30, head = doc.getElementsByTagName("head")[0] || docElem, base = doc.getElementsByTagName("base")[0], links = head.getElementsByTagName("link"), requestQueue = [], ripCSS = function () {
            for (var sheet, href, media, isCSS, sheets = links, sl = sheets.length, i = 0; sl > i; i++)sheet = sheets[i], href = sheet.href, media = sheet.media, isCSS = sheet.rel && "stylesheet" === sheet.rel.toLowerCase(), href && isCSS && !parsedSheets[href] && (sheet.styleSheet && sheet.styleSheet.rawCssText ? (translate(sheet.styleSheet.rawCssText, href, media), parsedSheets[href] = !0) : (!/^([a-zA-Z:]*\/\/)/.test(href) && !base || href.replace(RegExp.$1, "").split("/")[0] === win.location.host) && requestQueue.push({
                href: href,
                media: media
            }));
            makeRequests()
            ...
            var j, fullq, thisq, eachq, eql, qs = styles.match(/@media[^\{]+\{([^\{\}]*\{[^\}\{]*\})+/gi), ql = qs && qs.length || 0, href = href.substring(0, href.lastIndexOf("/")), repUrls = function (css) {
                return css.replace(/(url\()['"]?([^\/\)'"][^:\)'"]+)['"]?(\))/g, "$1" + href + "$2$3")
            }, useMedia = !ql && media, i = 0;
            for (href.length && (href += "/"), useMedia && (ql = 1); ql > i; i++)for (j = 0, useMedia ? (fullq = media, rules.push(repUrls(styles))) : (fullq = qs[i].match(/@media *([^\{]+)\{([\S\s]+?)$/) && RegExp.$1, rules.push(RegExp.$2 && repUrls(RegExp.$2))), eachq = fullq.split(","), eql = eachq.length; eql > j; j++)thisq = eachq[j], mediastyles.push({
    ...
    $("html").click(function () {
        $("a#responsive_menu_button").hasClass("responsive-toggle-open") && $(".js .main-nav .menu").slideToggle(function () {
            $("a#responsive_menu_button").removeClass("responsive-toggle-open"), $(".js .main-nav .menu").removeAttr("style")
        })
    })
}(jQuery), jQuery(".main-nav").click(function (event) {
